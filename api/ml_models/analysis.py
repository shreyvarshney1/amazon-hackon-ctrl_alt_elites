...
def analyze_review_text(text):
    ...
    final_score = (subjectivity_score * 0.7 +
                   (1 - sentiment_score) * 0.3) - spam_penalty
...
def check_image_authenticity(image_url):
    ...
    try:
        ...
        if annotations.pages_with_matching_images:
            # If the image is found on many other pages, especially stock photo
            # sites, it's a red flag.
        ...
        for entity in annotations.web_entities:
            if any(stock_word in entity.description.lower()
                   for stock_word in ["stock", "royalty-free", "illustration"]):
