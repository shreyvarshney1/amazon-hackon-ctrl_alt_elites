...
def calculate_scs_score(seller_id: int):
    ...
    seller_products_subquery = db.session.query(
        Product.id).filter(
        Product.seller_id == seller_id).subquery()
    seller_orders_query = db.session.query(Order).join(OrderItem).filter(
        OrderItem.product_id.in_(seller_products_subquery))
    ...
    # This analyzes reviews on the seller's products, looking for
    # seller-related feedback.
    all_seller_reviews = db.session.query(Review).join(
        Product).filter(Product.seller_id == seller_id).all()
    if all_seller_reviews:
        ...
        themes = analyze_aggregated_reviews(
            all_seller_reviews)  # Re-using stub for demo
    ...
    # We simulate it with a placeholder. A real implementation would query a
    # 'Disputes' table.
    ...
    # If a seller has no products or their PIS hasn't been calculated, give a
    # neutral score.
    ...
    # db.session.commit() is handled by the calling function
    # (recalculate_pis_and_cascade_scs)
    ...
    # 0=stable, 1=huge spike
    velocity_stability = 1 - data['sales_spike_factor']
