...
def calculate_uba_score(user_id: int) -> Optional[float]:
    ...
    unique_ips_count = db.session.query(distinct(UserSessionLog.ip_address))\
        .filter(UserSessionLog.user_id == user_id, UserSessionLog.timestamp >= thirty_days_ago)\
        .count()
    ...
    reviews_last_7d = db.session.query(Review.id)\
        .filter(Review.user_id == user_id, Review.created_at >= seven_days_ago).count()
    ...
    if recent_reviews:
        auth_scores = [
            r.linguistic_authenticity_score for r in recent_reviews
