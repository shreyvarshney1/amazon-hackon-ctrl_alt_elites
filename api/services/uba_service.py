...
def calculate_uba_score(user_id: int):
    ...
    unique_ips = db.session.query(
        func.count(
            distinct(
                UserSessionLog.ip_address))) .filter(
        UserSessionLog.user_id == user_id,
        UserSessionLog.timestamp >= thirty_days_ago).scalar()
    ...
    reviews_last_7d = db.session.query(func.count(Review.id)) .filter(
        Review.user_id == user_id, Review.created_at >= seven_days_ago).scalar()
    ...
    if recent_reviews:
        auth_scores = [
            r.linguistic_authenticity_score for r in recent_reviews if r.linguistic_authenticity_score is not None]
    ...
    # Assuming >10 unique IPs is max risk
    p2 = 1 - min(1, (data['unique_ips_last_30d'] /
                 10.0 + ip_analysis['proxy_risk']))

    # --- P3: Review Velocity (Weight: 25%) ---
    # `reviews_per_day` is the user's average
    # Assuming >5 reviews/day is max risk
    p3 = 1 - min(1, (data['reviews_per_day'] / 5.0))
